...
export async function fetchAdminOverview() {
  ...
  const [claims, profiles, documents, events, eligibility] = await Promise.all([
    client
      .from("claims")
  ...
  const errors = [claims, profiles, documents, events, eligibility].map((result) => result.error).filter(Boolean);
